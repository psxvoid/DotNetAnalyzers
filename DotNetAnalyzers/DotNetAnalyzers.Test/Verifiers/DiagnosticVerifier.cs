    ...
    public abstract partial class DiagnosticVerifier
    {
        ...
        private static void VerifyDiagnosticLocation(DiagnosticAnalyzer analyzer, Diagnostic diagnostic, Location actual, DiagnosticResultLocation expected)
        {
            ...
            Assert.IsTrue(
                ...
                (actualSpan.Path?.Contains("Test0.", StringComparison.InvariantCulture) ==
                    true && expected.Path.Contains("Test.", StringComparison.InvariantCulture)),
                string.Format(
                    CultureInfo.InvariantCulture,
                ...
                foreach (DiagnosticDescriptor rule in analyzer.SupportedDiagnostics)
                {
                    if (rule != null && rule.Id == diagnostics[i].Id)
                    {
                        ...
                        else
                        {
                            ...
                            string resultMethodName = diagnostics[i].Location.SourceTree.FilePath.EndsWith(".cs", System.StringComparison.InvariantCulture) ? "GetCSharpResultAt" : "GetBasicResultAt";
